...
esp_err_t bnn_spi_init(bnn_spi_t *iface, const bnn_spi_config_t *cfg)
{
    ...
    iface->tx_buf_tick = heap_caps_malloc(24, MALLOC_CAP_DMA);
    ...
    iface->trans_tick.length = 144; // 18 bytes
...
esp_err_t bnn_spi_tx_tick(bnn_spi_t *iface, uint32_t bid_price_q, uint32_t bid_qty_q, uint32_t ask_price_q, uint32_t ask_qty_q, uint8_t metadata)
{
    // Frame format: 144 bits (18 bytes)
    // [143:136] Control = 0x10
    // [135:104] Bid Price
    // [103:72]  Bid Qty
    // [71:40]   Ask Price
    // [39:8]    Ask Qty
